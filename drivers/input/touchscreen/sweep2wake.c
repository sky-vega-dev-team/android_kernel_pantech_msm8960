...
#define DEFAULT_S2W_PWRKEY_DUR          10
...
#if S2W_DEBUG
        pr_info(LOGTAG"touch_x,touch_y(%4d,%4d) single:%s\n",
                touch_x, touch_y, (single_touch) ? "true" : "false");
#endif
	if (s2w_swap_coord == 1) {
...
static int input_dev_filter(struct input_dev *dev) {
	if (strstr(dev->name, "touch")||strstr(dev->name, "sensor00fn11")||strstr(dev->name, "sensor00")||strstr(dev->name, "sensor00/fn11")||strstr(dev->name, "touch_monitor")||strstr(dev->name, "touch_fops")||strstr(dev->name, "pantech_touch")||strstr(dev->name, "pantech_touch")||strstr(dev->name, "rmi_generic")) {
		return 0;
	} else {
		return 1;
	}
